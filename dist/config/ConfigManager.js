...
// ---------------------------------------------------------------------------
function validateMunicipal(raw, index) {
    const errors = [];
    ...
    return {
        config: {
            ...
            documentTypeFilters: raw.documentTypeFilters,
            enabled: raw.enabled,
        },
        errors: [],
    };
}
function validateMarket(raw, index) {
    const errors = [];
    ...
    return {
        config: {
            ...
            authCredentialRef: raw.authCredentialRef,
            schedule: raw.schedule,
            enabled: raw.enabled,
        },
        errors: [],
...
export class ConfigManager {
    ...
    loadSources(filePath) {
        ...
        if (!Array.isArray(municipalRaw)) {
            ...
            for (let i = 0; i < municipalRaw.length; i++) {
                const result = validateMunicipal(municipalRaw[i], i);
        ...
        if (!Array.isArray(marketRaw)) {
            ...
            for (let i = 0; i < marketRaw.length; i++) {
                const result = validateMarket(marketRaw[i], i);
                if (result.config !== null) {
                    validSources.push(result.config);
                }
                else {
     ...
     */
    getActiveSources(type) {
        return this._sources.filter((s) => {
            if (!s.enabled)
                return false;
            if (type === "municipal")
                return "portalUrl" in s;
            if (type === "market")
     ...
     */
    watchForChanges(filePath, callback) {
        if (this._watcher) {
            this._watcher.close();
        }
