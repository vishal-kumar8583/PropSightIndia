...
function isStale(computedAt, thresholdMs) {
    return Date.now() - new Date(computedAt).getTime() > thresholdMs;
}
function buildZoneSummary(store, zoneId) {
...
function buildZoneDetail(store, zoneId) {
    const gvs = store.getGVSResult(zoneId);
    if (!gvs)
        return null;
    const trend = store.getTrendResult(zoneId);
...
function validateWeightSum(config) {
...
export async function handleRequest(req, res, store, engine) {
    ...
    if (method === "GET" && urlWithoutQuery === "/api/zones") {
        ...
        if (flagsParam) {
            const requestedFlags = flagsParam
                .split(",")
                .map((f) => f.trim())
                .filter((f) => f === "Appreciating" || f === "Undervalued");
            if (requestedFlags.length > 0) {
                summaries = summaries.filter((s) => requestedFlags.some((flag) => s.flags.includes(flag)));
            }
        }
