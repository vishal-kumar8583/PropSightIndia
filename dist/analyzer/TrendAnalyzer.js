export class TrendAnalyzer {
     ...
     */
    computePricingVelocity(history) {
        if (history.length < 3) {
            return { rtm: "Insufficient History", uc: "Insufficient History" };
        }
        const current = history[history.length - 1];
        const threeCyclesAgo = history[history.length - 3];
        const rtm = ((current.pricePerSqftRTM - threeCyclesAgo.pricePerSqftRTM) /
            threeCyclesAgo.pricePerSqftRTM) /
            3 *
            100;
        const uc = ((current.pricePerSqftUC - threeCyclesAgo.pricePerSqftUC) /
     ...
     */
    computeRentalYieldDelta(history) {
        if (history.length < 7) {
            return "Insufficient History";
        }
        const annualizedYield = (snap) => snap.rentalAbsorptionRate * 4;
        const current = history[history.length - 1];
        const currentYield = annualizedYield(current);
        const prior6 = history.slice(history.length - 7, history.length - 1);
        const mean = prior6.reduce((sum, snap) => sum + annualizedYield(snap), 0) / 6;
    ...
    rankZones(results) {
        ...
        const sorted = [...results].sort((a, b) => combinedMetric(b) - combinedMetric(a));
