...
const SIGNAL_RANGES: Record<keyof WeightConfig["weights"], { min: number; max: number }> = {
...
export class CorrelationEngine {
  private currentWeights: WeightConfig;
  private readonly minDataPoints: number;
  private readonly auditStore?: AuditStore;
  private readonly storedResults: Map<string, GVSResult> = new Map();
  private readonly storedLineage: LineageRecord[] = [];

  constructor(options: CorrelationEngineOptions) {
    if (!this.isWeightSumValid(options.initialWeights)) {
      throw new Error(
        "initialWeights sum must equal 1.0 (tolerance ε = 1e-9)"
      );
    }
  ...
  computeGVS(zone: Zone, signals: SignalBundle, weights?: WeightConfig): GVSResult {
    const activeWeights =
      weights !== undefined && this.isWeightSumValid(weights)
        ? weights
        : this.currentWeights;

    const totalDataPoints =
      signals.municipalDeclarations.length + signals.marketSnapshots.length;
    ...
    for (const signalName of Object.keys(activeWeights.weights) as Array<
      keyof WeightConfig["weights"]
    >) {
  ...
  projectTrajectory(options: {
    zone: Zone;
    currentGVS: number;
    historicalGVS: Array<{ monthOffset: number; score: number }>;
    horizon: number;
    ...
    const boostMap = new Map<number, string[]>();
    for (const decl of declarations) {
      if (decl.projectedCompletionDate == null) continue;
    ...
    // Build data points
    const dataPoints: GVSTimeSeries["dataPoints"] = [];
  ...
  recomputeAll(
    ...
    zoneSignals: Array<{ zone: Zone; signals: SignalBundle }>
  ): GVSResult[] {
    if (!this.isWeightSumValid(weights)) {
      return [];
    }
  ...
  private isWeightSumValid(weights: WeightConfig): boolean {
  ...
  private extractRawValues(
  ...
  ): Record<keyof WeightConfig["weights"], number> {
    const decls = signals.municipalDeclarations;
    const snaps = signals.marketSnapshots;

    const infrastructureTender = decls.filter(
      (d) => d.declarationType === "infrastructure_tender"
    ).length;
    ...
    const metroHighway = decls.some(
      (d) =>
        d.declarationType === "metro_line" ||
    ...
    const pricingVelocity =
      snapCount === 0
        ? 0
        : snaps.reduce(
            (acc, s) => acc + (s.pricePerSqftRTM + s.pricePerSqftUC) / 2,
            0
          ) / snapCount;
  ...
  private computeSlope(
    ...
    if (historicalGVS.length <= 1) return 0;
