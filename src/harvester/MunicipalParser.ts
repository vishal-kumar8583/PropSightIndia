...
type DeclarationType = MunicipalDeclaration["declarationType"];

function isValidDeclarationType(value: string): value is DeclarationType {
  return (VALID_DECLARATION_TYPES as readonly string[]).includes(value);
}
...
function parseHTML(content: string): Partial<{
  zoneId: string;
...
function parseJSON(content: string): Partial<{
  zoneId: string;
  declarationType: string;
  declaredDate: string;
  projectedCompletionDate: string | null;
...
function parsePDF(content: string): Partial<{
  zoneId: string;
  declarationType: string;
  declaredDate: string;
  projectedCompletionDate: string | null;
...
export class MunicipalParser {
  private readonly onDeclaration?: (decl: MunicipalDeclaration) => void;
  private readonly auditStore?: AuditStore;
  private readonly failedDocuments: string[] = [];
  ...
  parseDocument(raw: RawDocument): Result<MunicipalDeclaration, ParseError> {
    const contentStr =
      typeof raw.content === "string" ? raw.content : raw.content.toString("utf8");
    ...
    if (
      fields.projectedCompletionDate != null &&
    ...
    const declaration: MunicipalDeclaration = {
      id,
      sourceUrl: raw.url,
      zoneId: fields.zoneId,
      declarationType: fields.declarationType as DeclarationType,
      declaredDate: fields.declaredDate,
      projectedCompletionDate:
        fields.projectedCompletionDate != null && fields.projectedCompletionDate !== ""
          ? fields.projectedCompletionDate
          : null,
      rawDocumentRef: null,
  ...
  deduplicateDeclarations(records: MunicipalDeclaration[]): MunicipalDeclaration[] {
    const seen = new Map<string, MunicipalDeclaration>();
  ...
  async runCycle(source: MunicipalSourceConfig): Promise<IngestResult> {
    const errors: string[] = [];
    const declarations: MunicipalDeclaration[] = [];
      ...
      try {
        ...
        if (!response.ok) {
          ...
          if (attempt < MAX_RETRIES) {
            ...
            errors.push(
              `Source ${source.id} (${source.portalUrl}) marked unavailable after ${MAX_RETRIES} retries. Last error: ${lastError}`
            );
    ...
    let docContentType: RawDocument["contentType"] = "html";
